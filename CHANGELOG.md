...
### Features
- Added `/blindspot performanceMonitor` — toggles two live boss bars showing real-time performance data. Bar 1 shows total BlindSpot cost as ms/tick and tick-budget percentage alongside server MSPT and effective TPS (via Paper's ~5-second rolling average). Bar 2 breaks down per-subsystem cost (entities, block entities, scan blocks) with peak-spike tracking. Bar colors shift from green → yellow → red as budget usage increases. Bars 3 and 4 show real-time movement and look-rotation deltas vs the current `deltaTrackerSensitivity` thresholds — the fill represents how close you are to triggering a visibility recalculation, turning green with "RECALC" when the threshold is exceeded, or gray with "skip" when stationary. Requires `blindspot.timings` permission.
- Added `autoReload` config option (default `false`) — when enabled, BlindSpot watches `config.yml` for changes and automatically reloads after a 1-second delay. Pairs well with `performanceMonitor` for live-tuning config values without running `/blindspot reload` manually. Can be toggled on/off via manual reload; the watcher starts/stops accordingly.
...
- Added `closeTrackingRadius` for entities (default `0`) — when `requireCrouchToHide` is enabled, standing players within this radius are always visible (even through walls), while standing players beyond it require LOS. Crouching players always require LOS at any distance. Set to 0 to disable the distance gate (legacy behaviour where standing = always visible at any range).
...
### Bug Fixes
- Fixed masked players being removed from the tab list, preventing them from being referenced in commands like `/tp`. Player hiding now uses packet-level entity destruction (like item frames) instead of Bukkit's `hideEntity()` API, keeping players in the tab list while still hiding their rendered entity.
...
### Improvements
- Replaced `TickTimings` batch-and-snapshot model with a true rolling window using 20 one-second ring-buffer buckets. Old data rotates out continuously, so `amortizedAverageMs()` and `maxMs()` always reflect the most recent 20 seconds without stale jumps. Boss bars, `/blindspot timings`, and performance monitoring all use the same smooth, aligned timing source.
- Added `performance.timingsWindowSeconds` config option (default `20`) — controls the rolling-window length used by `TickTimings`. Shorter values (e.g. `5`) react faster for solo testing; longer values (e.g. `30–60`) smooth out spikes on busy servers. Applied on reload via service `restart()`.
